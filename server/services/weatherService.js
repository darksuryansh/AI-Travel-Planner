...
export const getWeatherForecast = async (location) => {
  try {
    ...
    // OneCall API 3.0 - 7 day forecast + current weather
    const url = `https://api.openweathermap.org/data/3.0/onecall`;
    
    const params = {
      lat: lat,
      lon: lng,
      exclude: 'minutely,hourly,alerts', // Exclude to save API costs
      units: 'metric',
    ...
    const daily = data.daily.slice(0, 7).map(day => ({
      ...
      pop: day.pop, // Probability of precipitation (0-1)
      rain: day.rain || 0,
      uvi: day.uvi,
      clouds: day.clouds // Cloud coverage %
    }));

    // Calculate optimal day (best weather score)
    ...
    // Fallback to free API if OneCall fails
    if (error.response?.status === 401 || error.response?.status === 403) {
      console.log('Falling back to free weather API...');
      return getFreeWeatherForecast(location);
    }
...
/**
 * Fallback: Get weather using free 5-day forecast API
 */
const getFreeWeatherForecast = async (location) => {
  try {
    const { lat, lng } = location;
    ...
    return {
      ...
      data: {
        ...
        location: { lat, lng },
        note: 'Using free API tier (5-day forecast)'
      }
    };

  } catch (error) {
    console.error('Free weather API error:', error.message);
    throw new Error(`Failed to get weather forecast: ${error.message}`);
  }
};
